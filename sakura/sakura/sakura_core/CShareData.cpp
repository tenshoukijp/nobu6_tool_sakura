...
//キーワード：ツールバー順序
		...
		m_pShareData->m_Common.m_bUseTaskTray = FALSE;				/* タスクトレイのアイコンを使う */
		m_pShareData->m_Common.m_bStayTaskTray = FALSE;				/* タスクトレイのアイコンを常駐 */
...
/************************/
	int nIdx = 0;
	int i;
	pShareData->m_Types[nIdx].m_nTextWrapMethod = WRAP_WINDOW_WIDTH;	// テキストの折り返し方法		// 2008.05.30 nasukoji
...
//#ifdef COMPILE_COLOR_DIGIT
		"半角数値",							TRUE,  FALSE, FALSE, RGB( 60, 60, 255 )		, RGB( 255, 251, 240 ),	//@@@ 2001.02.17 by MIK		//Mar. 7, 2001 JEPRO RGB(0,0,255)を変更  Mar.10, 2001 標準は色なしに
//#endif
	...
	//"シングルクォーテーション文字列", FALSE, FALSE, FALSE, RGB( 0, 0, 0 ), RGB( 255, 255, 255 ),
		"シングルクォーテーション文字列",	TRUE , TRUE, FALSE, RGB( 30, 30, 240 )	, RGB( 255, 251, 240 ),
		"ダブルクォーテーション文字列",		TRUE , TRUE, FALSE, RGB( 30, 30, 240 )		, RGB( 255, 251, 240 ),
...
//		pShareData->m_Types[11].m_ColorInfoArr[COLORIDX_URL].m_bDisp = FALSE;
	...
	//
	/* Lua*/
	pShareData->m_Types[16].m_cLineComment.CopyTo( 0, "--", -1 );		/* 行コメントデリミタ */
	pShareData->m_Types[16].m_cBlockComment.CopyTo( 0, "--[[", "]]" );	/* ブロックコメントデリミタ */
	pShareData->m_Types[16].m_nDefaultOutline = OUTLINE_TEXT;			/* アウトライン解析方法 */
	//シングルクォーテーション文字列を色分け表示しない
	pShareData->m_Types[16].m_ColorInfoArr[COLORIDX_SSTRING].m_bDisp = TRUE;
	//ダブルクォーテーション文字列を色分け表示しない
	pShareData->m_Types[16].m_ColorInfoArr[COLORIDX_WSTRING].m_bDisp = TRUE;
	//To Here Nov. 9, 2000

	/* リッチテキスト */
	pShareData->m_Types[15].m_nDefaultOutline = OUTLINE_TEXT;			/* アウトライン解析方法 */
	pShareData->m_Types[15].m_nKeyWordSetIdx[0]  = 15;					/* キーワードセット */
	pShareData->m_Types[15].m_ColorInfoArr[COLORIDX_DIGIT].m_bDisp = TRUE;	/* 半角数値を色分け表示 */
	pShareData->m_Types[15].m_nStringType = 0;							/* 文字列区切り記号エスケープ方法  0=[\"][\'] 1=[""][''] */
	//シングルクォーテーション文字列を色分け表示しない
	...
	//URLにアンダーラインを引かない
	pShareData->m_Types[15].m_ColorInfoArr[COLORIDX_URL].m_bDisp = FALSE;
	//To Here Jul. 10, 2001

	//From Here Nov. 9, 2000 JEPRO
	/* 設定ファイル */
	pShareData->m_Types[16].m_cLineComment.CopyTo( 0, "--", -1 );		/* 行コメントデリミタ */
	pShareData->m_Types[16].m_cLineComment.CopyTo( 1, "//", -1 );		/* 行コメントデリミタ2 */
	pShareData->m_Types[16].m_cLineComment.CopyTo( 2, ";", -1 );		/* 行コメントデリミタ3 */
	pShareData->m_Types[16].m_cBlockComment.CopyTo( 0, "--[[", "]]" );	/* ブロックコメントデリミタ */
	pShareData->m_Types[16].m_nDefaultOutline = OUTLINE_TEXT;			/* アウトライン解析方法 */
	//シングルクォーテーション文字列を色分け表示しない
	pShareData->m_Types[16].m_ColorInfoArr[COLORIDX_SSTRING].m_bDisp = TRUE;
