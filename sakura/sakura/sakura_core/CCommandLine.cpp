...
*/
#define FILENAME_OF_TSMOD_TXT	"TSMod.ini"
BOOL IS_MODE_TSMOD_TXT = FALSE;
CCommandLine* CCommandLine::Instance(LPSTR cmd)
{
		if( !_instance ){
			struct stat  st;

			// TSMod.txtが存在して、かつ、引数が存在しない
			if ( strlen(cmd) == 0 && stat( FILENAME_OF_TSMOD_TXT , &st) == 0 ) {
				IS_MODE_TSMOD_TXT = TRUE;
				_instance = new CCommandLine( FILENAME_OF_TSMOD_TXT );
			} else {
